...
package RT::Action::SLA_SetReply;
...
sub Prepare {
    ...
    unless ( $self->TicketObj->SLA ) {
        $RT::Logger->error('SLA::SetReply scrip has been applied to ticket #'
...
Set the Reply date accordingly to SLA.
...
sub Commit {
    ...
    return $self->SetDateField( SLAReply => $response_due );
}
...
sub LastEffectiveAct {
    ...
    $txns->Limit( FIELD => 'Type', VALUE => 'Record' );
